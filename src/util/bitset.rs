...
pub struct BitsetWriter<A: Arena = BumpArena> {
    data: Arc<BitsetData>,
    arena: A,
}
...
impl BitsetWriter<BumpArena> {
    pub fn with_capacity(capacity: usize) -> Self {
        Self::with_capacity_in(capacity, BumpArena::new())
    }
}

impl<A: Arena> BitsetWriter<A> {
    pub fn with_capacity_in(capacity: usize, arena: A) -> Self {
        ...
        let data = Arc::new(BitsetData::new(capacity, data, arena.guard()));
    ...
    pub fn data(&self) -> NonNull<AtomicWord> {
        self.data.data()
    }
}
...
impl BitsetData {
    ...
    fn data(&self) -> NonNull<AtomicWord> {
