...
impl From<Vec<i8>> for OwnedValue {
    fn from(values: Vec<i8>) -> OwnedValue {
...
impl From<Vec<String>> for OwnedValue {
    fn from(values: Vec<String>) -> OwnedValue {
        OwnedValue::Array(values.into_iter().map(|v| v.into()).collect())
    }
}
