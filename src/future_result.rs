...
enum Inner<T> {
    FailedBeforeStart(Option<TansakuuError>),
    InProgress {
...
impl<T> From<TansakuuError> for FutureResult<T> {
    fn from(err: TansakuuError) -> Self {
        FutureResult {
    ...
    pub fn wait(self) -> crate::Result<T> {
        match self.inner {
            ...
            Inner::InProgress {
                ...
                Err(crate::TansakuuError::SystemError(
...
//     use crate::TansaError;

//     #[test]
//     fn test_scheduled_result_failed_to_schedule() {
//         let scheduled_result: FutureResult<()> = FutureResult::from(TansaError::Poisoned);
//         let res = block_on(scheduled_result);
//         assert!(matches!(res, Err(TansaError::Poisoned)));
//     }
...
//         drop(tx);
//         let res = block_on(scheduled_result);
//         assert!(matches!(res, Err(TansaError::SystemError(_))));
//     }
...
//         let (scheduled_result, tx): (FutureResult<u64>, _) = FutureResult::create("failed");
//         tx.send(Err(TansaError::Poisoned)).unwrap();
//         let res = block_on(scheduled_result);
//         assert!(matches!(res, Err(TansaError::Poisoned)));
//     }
// }
